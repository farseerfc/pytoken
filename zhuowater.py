...
def suffix_tree(str): 
    ...
    for i in range(len(str)): 
        ...
        print 'finish updata ',node,l
        (node, l) = canonize(t, node, (l, i))
        print 'char in ',node.children,l
    return t 
# 
...
def canonize(t, node, str_ref): 
    (l, r) = str_ref 
    print 'canoize node=',node,'pair=',str_ref
    if node is None: 
        if length(str_ref)<=0: 
            return (None, l) 
        else: 
...
def to_lines(t, node): 
    ...
    for c, (str_ref, tr) in sorted(node.children.items()): 
        ...
        if len(node.children)>1: 
            lines[1:] = map(lambda l: "|"+" "*(len(edge_str)+5)+l, 
lines[1:]) 
        else: 
            lines[1:] = map(lambda l: " "+" "*(len(edge_str)+6)+l, 
lines[1:]) 
        if res !=[]: 
            res.append("|") 
        res += lines 
